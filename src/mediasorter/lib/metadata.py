...
class MetadataApi:
    ...
    async def query(self, *args, **kwargs) -> TvShowMetadata | MovieMetadata:
    ...
    async def try_harder(
        ...
        raise to_be_raised or MetadataQueryError(
    ...
    async def _match_tv_show(
        ...
        def find_episode(e_list):
            for episode in e_list:
                if (
                    episode.get("season") == season_id
                    and episode.get("number") == episode_id
                ):
                    return episode
        ...
        if not correct_episode:
            ...
            if series_data.get("_links") and series_data["_links"].get("self"):
                alternative_url = (
                    series_data["_links"]["self"]["href"] + "/episodes?specials=True"
                )
                ...
                if not correct_episode:
                    ...
                    season_only = list(
                        filter(lambda e: e["season"] == season_id, episode_list)
                    )
                    if not season_only:
                        msg = (
                            f"TV show {series_data.get('name')} found, BUT "
        ...
        if not correct_episode:
            ...
            msg = (
                f"TV show {series_data.get('name')} found, BUT correct "
...
class TMDB(MovieMetadataApi):
    ...
    def clean_search_term(self, string, overrides: dict[str, str] | None = None):
        ...
        if overrides and (search_movie_title in overrides):
            ...
            logger.debug(
                f"Overriding search title: '{search_movie_title}' -> '{new_name}'"
            )
    ...
    async def _match_movie(
        ...
        if search_year:
            new_result_list = list(
                filter(
                    lambda res: abs(self._parse_release_year(res) - search_year) < 2,
                    result_list,
                )
            )
        elif len(new_result_list) > 2:
            new_result_list = [
                item for item in result_list if item.get("popularity", 100) > 10
            ]
